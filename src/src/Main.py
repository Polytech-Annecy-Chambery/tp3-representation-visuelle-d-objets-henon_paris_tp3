...
def Q3a():
    wall=Wall({'position':[1,1,0],'width':7,'height':2.6,'orientation':90})
    Configuration().add(wall).display()

def Q4a():
    # Ecriture en utilisant des variables : A compléter
    wall1 = Wall({'position':[0,0,0],'width':7,'height':2.6,'orientation':90})
    wall2 = Wall({'position':[0,0,0],'width':7,'height':2.6,'orientation':0})
